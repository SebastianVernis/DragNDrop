...
class LayersManager {
    ...
    _buildLayerNode(element, parent, depth) {
        ...
        const className = element.className.replace('canvas-element', '').replace('selected', '').trim();
     ...
     */
    getSelectedLayers() {
        return Array.from(this.selectedLayers).map(id => this.getLayer(id)).filter(Boolean);
    }
